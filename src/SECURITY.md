...
*   **Server Actions**: All data mutations and AI-driven actions (e.g., submitting a proposal, generating a document, approving a decision) are handled via Next.js Server Actions. These are RPC-style calls that execute securely on the server, meaning the client never has direct access to the database or business logic.
...
The security and governance of AI interactions are paramount, especially when processing sensitive proposal documents and prompts.

*   **Server-Side Execution**: All generative AI features—from document scaffolding and ingestion to vetting and meeting summaries—are powered by **Genkit**, which runs exclusively on the server. AI `flows` are defined and executed as server-side functions.
...
*   **Data Flow Example (Intelligent Ingestion)**: When a user analyzes a document, the flow is as follows:
    1.  The client invokes a Server Action. For this prototype, the action reads a sample document from the server's file system to simulate an upload.
    2.  The server-side Genkit flow sends the document's content to the configured Google Cloud LLM endpoint (which can also be regional).
    3.  The LLM's response (suggested title, background, etc.) is processed by the Genkit flow on the server.
    4.  The final, structured output is returned to the client and populated into the form.
...
*   **Benefit**: This architecture ensures that sensitive proposal data is never exposed to the client's browser, API keys are kept secure, and your data is not used to train external models.
