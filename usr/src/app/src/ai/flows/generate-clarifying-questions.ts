...
const clarifyGoalFlow = ai.defineFlow(
  ...
  async (input) => {
    console.log('AGENT (clarifyGoalFlow): Received goal. Generating clarifying questions...');
    console.log(`AGENT (clarifyGoalFlow): Goal: "${input.userGoal}"`);
    
    const { output } = await prompt(input);

    if (!output?.questions || output.questions.length === 0) {
      console.error('AGENT (clarifyGoalFlow): Failed to generate questions from LLM.');
    ...
    output.questions.forEach(q => console.log(`  - [${q.category}]: ${q.question}`));
