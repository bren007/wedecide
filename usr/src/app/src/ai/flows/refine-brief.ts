...
import type { RefineBriefInput, RefineBriefOutput, BriefContent, FullArtifactContent } from '@/lib/types';
...
const refineBriefFlow = ai.defineFlow(
  ...
  async (input) => {
    console.log(`AGENT (refineBriefFlow): Received instruction. Refining document...`);
    ...
    if (!output) {
      console.error('AGENT (refineBriefFlow): Failed to generate refined document from LLM.');
      throw new Error('The agent failed to refine the document.');
    }
    
    console.log('AGENT (refineBriefFlow): Successfully refined document. New brief title:', output.brief.title);
    return output;
  }
);
